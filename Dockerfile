...
ENV DJANGO_SETTINGS_MODULE=scoresnow.settings.production DJANGO_DEBUG=off

# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=scoresnow/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/scoresnow/media/"
...
RUN mkdir -p /code/scoresnow/media/images && chown -R 1000:2000 /code/scoresnow/media

# mark the destination for images as a volume
VOLUME ["/code/scoresnow/media/images/"]
