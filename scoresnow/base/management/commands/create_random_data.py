...
from scoresnow.base.models import FooterText, HomePage, People, StandardPage
from scoresnow.blog.models import BlogIndexPage, BlogPage
from scoresnow.series.models import (
    SeriesIngredient,
    SeriesPage,
    SeriessIndexPage,
    SeriesType,
...
from scoresnow.teams.models import TeamsPage, TeamsIndexPage
...
class Command(BaseCommand):
    ...
    def create_pages(self, page_count):
        self.stdout.write("Creating series pages...")
        series_index = SeriessIndexPage.objects.live().first()
        for _ in range(page_count):
            title = self.make_title()
            series_index.add_child(
                instance=SeriesPage(
                    title=title,
                    slug=slugify(title),
                    introduction=lorem_ipsum.paragraph(),
                    series_type=self.get_random_model(SeriesType),
        ...
        self.stdout.write("Creating team pages...")
        teams_index = TeamsIndexPage.objects.live().first()
        for _ in range(page_count):
            title = self.make_title()
            teams_index.add_child(
                instance=TeamsPage(
                    title=title,
                    slug=slugify(title),
                    introduction=lorem_ipsum.paragraph(),
    ...
    def create_snippets(self, snippet_count):
        ...
        self.stdout.write("Creating series ingredients...")
        for _ in range(snippet_count):
            SeriesIngredient.objects.create(name=self.make_title())

        self.stdout.write("Creating series types...")
        for _ in range(snippet_count):
            SeriesType.objects.create(title=self.make_title())
