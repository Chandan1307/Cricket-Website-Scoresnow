...
from scoresnow.teams.choices import DAY_CHOICES


class OperatingHours(models.Model):
    """
    A Django model to capture operating hours for a Teams
    """
    ...
    closed = models.BooleanField(
        "Closed?", blank=True, help_text="Tick if team is closed on this day"
    )
...
class TeamsOperatingHours(Orderable, OperatingHours):
    """
    A model creating a relationship between the OperatingHours and Teams
    Note that unlike BlogPeopleRelationship we don't include a ForeignKey to
    OperatingHours as we don't need that relationship (e.g. any Teams open
    ...
    name to access it from the TeamsPage admin
    """

    team = ParentalKey(
        "TeamsPage", related_name="hours_of_operation", on_delete=models.CASCADE
    )
...
class TeamsIndexPage(Page):
    ...
    # Only TeamsPage objects can be added underneath this index page
    subpage_types = ["TeamsPage"]
    ...
    def get_context(self, request):
        context = super(TeamsIndexPage, self).get_context(request)
        context["teams"] = (
            TeamsPage.objects.descendant_of(self).live().order_by("title")
        )
        return context
...
class TeamsPage(Page):
    """
    Detail for a specific bakery team.
    """
    ...
    # Determines if the team is currently open. It is timezone naive
    def is_open(self):
        now = datetime.now()
        ...
        try:
            ...
            return True
        except TeamsOperatingHours.DoesNotExist:
            return False
    ...
    def get_context(self, request):
        context = super(TeamsPage, self).get_context(request)
        context["lat"] = self.lat_long.split(",")[0]
        context["long"] = self.lat_long.split(",")[1]
        context["google_map_api_key"] = settings.GOOGLE_MAP_API_KEY
        return context

    # Can only be placed under a TeamsIndexPage object
    parent_page_types = ["TeamsIndexPage"]
